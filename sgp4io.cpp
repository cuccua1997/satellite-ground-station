...
void twoline2rv
     (
      ...
      char      shortstr11[32], char shortstr12[37], char shortstr21[33], char shortstr22[36],
     ...
     )
     {
       ...
       for (k = 0; k<=31; k++){
         shortstr11[k] = longstr1[k];
       };
       for (l = 0; l<=36; l++){
         shortstr12[l] = longstr1[32+l];
       };
       for (m = 0; m<=31; m++){
         shortstr21[m] = longstr2[m];
       };
       for (n = 0; n<=36; n++){
         shortstr22[n] = longstr2[32+n];
       };

      sscanf(shortstr11,"%2d %5ld %1c %10s %2d %12lf",&cardnumb,&satrec.satnum,&classification, intldesg, &satrec.epochyr, &satrec.epochdays);
      sscanf(shortstr12,"%11lf %7lf %2d %7lf %2d %2d %6ld",&satrec.ndot, &satrec.nddot, &nexp, &satrec.bstar,&ibexp, &numb, &elnum);
      sscanf(shortstr21,"%2d %5ld %9lf %9lf %8lf",&cardnumb,&satrec.satnum, &satrec.inclo,&satrec.nodeo,&satrec.ecco);
      sscanf(shortstr22,"%9lf %9lf %10lf %6ld %lf %lf %lf",&satrec.argpo, &satrec.mo, &satrec.no, &revnum, &startmfe, &stopmfe, &deltamin);
